...
weight_path = 'biformer_b_bin_' + str(batch_size) + '_0202'+'.pth'
...
# 加载预训练的biformer模型
model = biformer_base(pretrained=True)  # 加载预训练的模型
model.head = nn.Sequential(nn.Linear(model.head.in_features, 100),
                           nn.Dropout(0.5),
                           nn.Linear(100, 1),
